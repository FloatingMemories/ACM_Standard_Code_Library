...
//		lazy2[rt] = INF;
	...
	static void add_range(LL &rt ,LL l, LL r, LL L, LL R, LL val){
		if(!rt){
			rt = ++tot;
//			minn[rt] = INF;
//			lazy2[rt] = INF;
...
//			lazy2[rt] = min(lazy2[rt], val);
		...
		push_down(rt, r - l + 1);
		LL mid = l + r >> 1;
		if(mid >= L) add_range(lson[rt], l, mid, L, R, val);
		if(mid < R) add_range(rson[rt], mid + 1, r, L, R, val);
		push_up(rt);
	}
	
	static void add_point(LL &rt, LL l, LL r, LL pos, LL val){
		if(!rt){
			rt = ++tot;
//			minn[rt] = INF;
		}
...
//			minn[rt] = min(minn[rt], val);
		...
		LL mid = l + r >> 1;
		if(mid >= pos) add_point(lson[rt], l, mid, pos, val);
		else add_point(rson[rt], mid + 1, r, pos, val);
		push_up(rt);
	}
...
void solve(){
	LL rt = 0, l = -1e9, r = 1e9; // 根（必需为0）和值域（操作范围）
	SGT::root = rt;
	SGT::add_point(rt, l, r, 1e4, 1e4);
	SGT::add_range(rt, l, r, 100000, 100010, 100);
	cout << SGT::query(rt, l, r, -1e9, 0) << '\n';
	cout << SGT::query(rt, l, r, 0, 1e5) << '\n';
	cout << SGT::query(rt, l, r, 1e5, 1e6) << '\n';
	cout << SGT::query(rt, l, r, 1e6, 1e10) << '\n';
}
```
