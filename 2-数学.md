...
struct Median{
	...
	multiset<LL> smaller,bigger;
	LL mid = -inf;
	int sz = 2;
	
	void init(){
		smaller.clear();bigger.clear();
		mid = -inf;
		smaller.insert(-inf);
		bigger.insert(inf);
		sz = 2;
	...
	void add(LL val){
		if(sz & 1){ // odd add
			...
			if(val >= *smaller.rbegin() && val <= *bigger.begin()){
				mid = val;
			}else if(val < *smaller.rbegin()){
				smaller.insert(val);
				mid = *smaller.rbegin();
	...
	void erase(LL val){
		if(sz & 1){
			...
			if(val <= *smaller.rbegin())
			{
				smaller.erase(smaller.find(val) );
				smaller.insert(mid);
				mid = *smaller.rbegin();
			}else if(val >= *bigger.begin()){
...
- 解决如下卷积问题
- 公式：$C_{i} = \sum_{i=j \oplus k}A_{j} B_{k}$（其中 $\oplus$ 是二元位运算中的某一种）
...
+ 错排公式（对于 $1\sim n$ 的排列 $P$，满足 $P_i\neq i$）：$D_1=0,D_2=1,D_n=(n-1)(D_{n-1} + D_{n-2})=n!(\frac 1{2!}-\frac 1{3!}+\dots + (-1)^n\frac 1{n!})=\lfloor \frac{n!}e + 0.5 \rfloor$
